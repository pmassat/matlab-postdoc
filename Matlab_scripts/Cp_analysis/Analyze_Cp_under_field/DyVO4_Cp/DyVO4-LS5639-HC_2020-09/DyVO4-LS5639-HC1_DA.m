...
%% Data importation
sample = 'DyVO4-LS5639-HC1';
filename = '2020-09-25_DyVO4-LS5639-HC1.dat';
cd 'C:\Users\Pierre\Desktop\Postdoc\DyVO4\DyVO4_Cp\DyVO4-LS5639-HC_2020-09'
Data1=ImportTmVO4Cp(filename);% Use this data to plot color map of phase diagram

%% Assign data to variables
H=[Data1.FieldOersted];%*rescaling;
T=[Data1.SampleTempKelvin];
Cp=[Data1.SampHCJmoleK];
CpErr=[Data1.SampHCErrJmoleK];
...
clear separatedHC1CpData
for i = 1:length(uhrf)
    wp = abs(H-uhrf(i))<50;
    separatedHC1CpData(i).H = H(wp);
    separatedHC1CpData(i).T = T(wp);
    separatedHC1CpData(i).Cp = Cp(wp);
    separatedHC1CpData(i).d1Cp = d1Cp(wp);
    separatedHC1CpData(i).CpErr = CpErr(wp);
    
    [separatedHC1CpData(i).T,wo] = sort(separatedHC1CpData(i).T);
    separatedHC1CpData(i).H = separatedHC1CpData(i).H(wo);
    separatedHC1CpData(i).Cp = separatedHC1CpData(i).Cp(wo);
    separatedHC1CpData(i).d1Cp = separatedHC1CpData(i).d1Cp(wo);
    separatedHC1CpData(i).CpErr = separatedHC1CpData(i).CpErr(wo);
end
...
avgHC1Data(i) = averageCpwithH2(Tsep,separatedHC1CpData(i).T,separatedHC1CpData(i).Cp,...
    separatedHC1CpData(i).CpErr,separatedHC1CpData(i).H);
end
for i=length(uhrf):-1:1
avgHC1Data(i).Cpel = avgHC1Data(i).Cp - R*(avgHC1Data(i).T/Tp).^3;% electronic contribution to Cp, after subtracting phonons contribution
avgHC1Data(i).Cpelr = avgHC1Data(i).Cpel/R;
avgHC1Data(i).CpelrErr = avgHC1Data(i).CpFullErr/R;
avgHC1Data(i).uh = unique(round(avgHC1Data(i).H,-2));
end

%% Plot averaged data 
figure; hold on
for i = 1:length(uhrf)
plot(avgHC1Data(i).T,avgHC1Data(i).Cp,'.-', 'MarkerSize', 12, 'LineWidth',1,...
    'DisplayName',sprintf('%i kOe',uhrf(i)/10^3))
...
legend('show')
title('DyVO$_4$-LS5639-HC1 heat capacity')
xlabel('T (K)'); ylabel('$C_p$ (uJ/K)');
xlim([0 30])
% zoom
...
for i=1:length(uhrf)
    [M(i),I] = min(avgHC1Data(i).d1Cp);
    Tcd1(i) = avgHC1Data(i).T(I);
end

%% Prepare MF fit of Cp vs Temperature at given field
j=1;
Tfit = avgHC1Data(j).T;
Cpfit = avgHC1Data(j).Cp;
CpfitErr = avgHC1Data(j).CpFullErr;
wghts = 1./CpfitErr;
Tmaxfit = 2.15;
...
fitT = avgHC1Data(i).T;
fitCp = avgHC1Data(i).Cpelr;
fitCpErr = avgHC1Data(i).CpFullErr/R;
fitwghts = 1./fitCpErr;
% Tc = 2.15;
...
plot_Cp_avg_w_fits(rngPlot, avgHC1Data, CpnumRF, Tc0rf, uhrf/Hc)
...
i = 5;
Hdata = unique(round(avgHC1Data(i).H,-2));
...
% idx = 8
    ...
    Hext_data = unique(round(avgHC1Data(i).H,-1));
...
%     toc
    ...
    % Identify spikes
    spikes = abs(Cpwewt)>1.2*max(avgHC1Data(i).Cpelr) | Cpwewt<0;% 
...
for idx=1%:length(rngMFD)
    ...
    eb{rngNum==i} = errorbar(avgHC1Data(i).T,avgHC1Data(i).Cpelr,avgHC1Data(i).CpelrErr,...
...
figure
plot(avgHC1Data(i).T,avgHC1Data(i).Cpelr,'.','DisplayName','data')
hold on;
...
for i=rng
eb{rng==i} = errorbar(avgHC1Data(i).T,avgHC1Data(i).Cpelr,avgHC1Data(i).CpFullErr/R,...
...
index = 15;
H1 = uhrf(index);
T1 = avgHC1Data(index).T;
Cp1 = avgHC1Data(index).Cp/R;
Cp1Err = avgHC1Data(index).CpFullErr/R;
wghts1 = 1./Cp1Err;
...
figure
for i=1:length(uhrf)
    scatter3(separatedHC1CpData(i).H,separatedHC1CpData(i).T,separatedHC1CpData(i).Cp,'.',...
...
figure
for i=1:length(uhrf)
    scatter3(separatedHC1CpData(i).H,separatedHC1CpData(i).T,-separatedHC1CpData(i).d1Cp,'.',...
        'Displayname',num2str(uhrf(i)', '%d Oe'))
%     plot(separatedCpData(i).T,separatedCpData(i).Cp,'-+')
    hold on
end
...
figure
for i=1:length(uhrf)
    plot(separatedHC1CpData(i).T,separatedHC1CpData(i).Cp,'-+')
    hold on
end
