...
for j=1:20
    am5(j) = mean(alpha5(1:j));
    stdam5(j) = std(alpha5(1:j));
end 
sm5 = mean(sigma5(1:10));% same with sigma5, which only depends on the instrument resolution
%% Recompute fit with fixed sigma parameter
for i=istart:-1:iend
    [nDatap94(i).fit4rslt, nDatap94(i).gof4] = ENS_peak_fit_ICpV_function_4params(...
        nDatap94(i).hh0,nDatap94(i).I,hc);
    label = strcat("T=",num2str(round(nDatap94(i).temp,2)),"K & H=",...
        num2str(nDatap94(i).field),"T");
    if mod(i,5)==2% plot data every 5 fields
        ENS_peak_fit_plot(nDatap94(i).hh0,nDatap94(i).I,hc,nDatap94(i).fit4rslt)
        title(strcat("ENS pattern cut along [hh0] at ",label))
        xlim([hc-.25 hc+.25]);
    end
...
%% Recompute fit with fixed alpha parameter
for i=istart:-1:iend
    ...
    label = strcat("T=",num2str(round(nDatap94(i).temp,2)),"K & H=",...
        num2str(nDatap94(i).field),"T");
    if mod(i,5)==2% plot data every 5 fields
        ENS_peak_fit_plot(nDatap94(i).hh0,nDatap94(i).I,hc,nDatap94(i).fit3rslt)
        title(strcat("ENS pattern cut along [hh0] at ",label))
        xlim([hc-.25 hc+.25]);
    end
