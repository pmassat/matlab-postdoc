function [Cp,sz,dsz] = Cp_sz_dsz_random_strains(d0,t,x)
...
for k=1:length(t)
    sz(k) = order_parameter_random_strains(d0,t(k),x);
    try
    dsz(k) = order_parameter_derivative_random_strains(d0,t(k),sz(k),x);
    catch
        disp(k);
        break
    end
    ...
    Cp(k) = x/sqrt(pi)*integral(@(u)exp(-(u).^2).*Cpintegrand(u),-Inf,Inf,'ArrayValued',true);
